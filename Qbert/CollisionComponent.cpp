...
void dae::CollisionComponent::BeginPlay()
{
	...
	//order matters because how can I subscribe to a componentn that is  innitilaized later 
	std::cout << " Collision Component Initiled\n";
...
void dae::CollisionComponent::Update()
{
	...
	if (tagComponent)
	{
		if (tagComponent->GetTag() != "SnakePlayer")  //not snake so Player 
		{
...
void dae::CollisionComponent::Dificulty2(SDL_Rect& selfRect, int i)
{
	...
	if (Collides(selfRect, m_Map[i]->GetComponent<dae::RectangleComponent>()->GetRect()))
	{
		...
		if (m_Map[i]->GetComponent<dae::RenderComponent>()->m_CanCollide)
		{
			...
			if (m_Map[i]->GetComponent<dae::RenderComponent>()->m_state != 5)  //2
			{
	...
	else
	{
		...
		//set variable can add score bCK TO TRUE
...
void  dae::CollisionComponent::OpenScene(const std::string& scenename)
{
	...
	if (newScene->GetName() == "InputNameScene")  return;
